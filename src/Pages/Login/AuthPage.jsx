...
export default function AuthSlidePanel() {
  ...
  const handleSignup = async (e) => {
    ...
    try {
      ...
      if (signup.Account_type === 'Seller Account') {
        res = await axios.post('http://localhost:5000/api/sellers/signup', {
          name: signup.name,
          email: signup.email,
          phone: signup.phone,
        ...
        res = await axios.post('http://localhost:5000/api/customers/signup', {
          name: signup.name,
          email: signup.email,
          phone: signup.phone,
          password: signup.password,
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      // ✅ Attach account type to returned user
      const userWithType = {
        ...res.data.user,
        loginType: loginType,
      };
