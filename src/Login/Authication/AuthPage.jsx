...
export default function AuthSlidePanel() {
  ...
  const handleSignup = async (e) => {
    ...
    try {
      // if seller account selected
      if (signup.Account_type === 'Seller Account') {
        const res = await axios.post('http://localhost:5000/api/sellers/signup', {
          name: signup.name,
          email: signup.email,
          phone: signup.phone,
      ...
      setIsSignIn(true);
      setSignup({ name:'', email:'', phone:'', password:'', confirm:'', Account_type:'Customer Account', image:null });
    } catch (err) {
      ...
      <div className="relative w-full max-w-5xl h-[500px] bg-white rounded-3xl shadow-xl overflow-hidden grid grid-cols-2">
        ...
        <div className="flex items-center justify-center bg-white z-10">
          ...
          >
            ...
            <Input icon={<User />} placeholder="Phone" value={signup.phone} onChange={e => setSignup({ ...signup, phone: e.target.value })} />
        ...
        <div className="flex items-center justify-center bg-white z-10">
          ...
          >
           ...
           <select className="w-full py-2 px-4 rounded-full border focus:ring-2 focus:ring-[#7D5FFF]"
            value={loginType}
